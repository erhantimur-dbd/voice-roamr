    ...
    <SiteShell>
      ...
      <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
        <div className="flex flex-wrap gap-2">
          {VOICE_TYPES.map((v) => (
            <span key={v.id} className="rounded-full border border-border px-3 py-2 text-sm text-muted">
              {v.label}
            </span>
          ))}
        </div>
        <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {VOICES.map((v) => (
            <article key={v.id} className="rounded-[20px] border border-border bg-surface p-5">
              ...
              <p className="mt-3 text-xs text-primary">Best for {v.best}</p>
            </article>
          ))}
        </div>
      </div>
